...
## Local Research Session Assistant
...
- use approved/fake linked-source cache paths only; no live HTTP fetch is run by
  default;
...
- print a privacy/cost receipt with `model_calls=0`,
  `estimated_cost_usd=0`, no provider egress, no live linked-source fetch, and
...
### Monday Morning - Scheduled Report V2 Package

Schedule:
- `telegram-ai-split-report.timer`: Monday 09:00 `Asia/Tbilisi`

The systemd timer refreshes Telegram ingestion and runs:

```bash
python3 src/main.py weekly-intelligence-v2 --deliver --threads-limit 24 --atoms-limit 8
```

The current timer may deliver:
- Telegram text notification with the finalized weekly run status.
...
- Telegram document: `Knowledge Atlas` HTML for cumulative trend/source/study
  context.
- Same-run Radar state inside the manifest-bound package. A separately
  generated Radar filename is not sufficient proof of package membership.
- Legacy Research Brief, Implementation Ideas, Study Plan, and visual report
  ...
  outputs.
- When run on demand, local package files under
  `data/output/weekly_intelligence_runs/<run_id>/`.

This is the scheduled V2 package command, but dogfood evidence still starts
only after `report-v2-rollout-gate` returns `eligible` on the real package.
The service writes the latest post-run gate receipt to
`data/output/report_v2_rollout_receipts/latest.json`; a blocked receipt is a
fix list, not dogfood evidence.
Weekly Brief V2 must be a 5-7 minute Russian operational read; Knowledge Atlas
V2 must be a separate visual cumulative map. See
...
### Target Weekly Brief / Knowledge Atlas Routine

The Report V2 target reader-facing flow has two surfaces plus one technical
surface:
...
- `Knowledge Audit Explorer`: open only from technical details when atom,
  evidence, raw membership, ID, or source-construction inspection is required.
...
### Current V1 Deployment Diagnostics

These commands inspect the deployed V1 services. They do not authorize dogfood
or prove Report V2 readiness:
