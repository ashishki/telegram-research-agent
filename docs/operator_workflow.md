...
- Telegram document: `Knowledge Atlas` HTML for cumulative trend/source/study
  context.
- An optional/on-demand Radar candidate dossier generated by a separate run.
  Filename adjacency does not prove that it belongs to the same report period.
...
- `telegram-ai-split-report.timer` should be `active` and `enabled`; it is the
  current diagnostic weekly report schedule and should trigger Monday at 09:00
  Europe/Berlin.
