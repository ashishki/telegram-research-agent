...
Acceptance-Criteria:
  ...
  - The existing durable callback uses additive `prm_post_answer_action_binding.v1`: `context_kind=prm`, context ID, exact canonical SHA-256 source snapshot, optional `{origin: answer.project_name, value}` provenance, offered action codes, private owner chat/actor hashes and expiry. Controls require positive `actor_id == chat_id == owner_chat_id`; draft and confirmation validate all binding fields and the requested action before they can create or confirm a proposal. Missing/pre-binding/tampered/mismatched rows fail closed without a write or reroute.
  - `test_post_answer_controls_require_private_owner_actor_binding` and `test_post_answer_context_binds_canonical_snapshot_and_project_ref` prove the bound positive path after mutable dialog changes; `test_post_answer_callback_preserves_bound_source_without_reroute`, `test_post_answer_context_rejects_expired_wrong_chat_actor_or_tampered_binding`, and `test_plain_language_action_selection_rejects_stale_or_cross_topic_context` prove preservation and denial.
