...
Acceptance-Criteria:
  ...
  - `encode_utd_proposal_state`/`decode_utd_proposal_state` are the sole state codec. Onboarding, draft load/save/discard, profile preview/confirm/cancel and subscription start/claim/finish/cancel are mandatory callers and test targets; a production-schema integration test executes every transition.
