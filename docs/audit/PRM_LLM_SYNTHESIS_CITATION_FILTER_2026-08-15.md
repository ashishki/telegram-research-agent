...
The path is fail-closed: only JSON `{"verdict":"pass"}` permits the generated
text. A provider failure, invalid JSON, or any other verdict returns the
existing deterministic professional answer renderer instead. Both synthesis
and filter run inside usage-recording suppression.
...
overstated independence claim; both were corrected. Re-review found no P0/P1
