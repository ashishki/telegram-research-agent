...
def main() -> int:
    ...
    print(json.dumps({"enabled": result.enabled, "profile_loaded": result.profile_loaded, "source_status": dict(result.source_status), "change_count": len(result.changes), "relevant_change_count": sum(bool(x.get("relevance", {}).get("relevant")) for x in result.changes)}, ensure_ascii=False, sort_keys=True))
    return 0
